...
def insertDataToPostgres():
    try:
        df = pd.read_csv('./chunk-data.csv')
        df.columns = [c.lower() for c in df.columns]  # PostgreSQL doesn't like capitals or spaces
        df.rename(columns=lambda x: x.replace(".", "_"), inplace=True)
        df1 = pd.read_csv('./relation__cityGroup.csv')
        ...
        engine = create_engine('postgresql://admin:admin@db:5432/GLEIF')
        df.to_sql("Entity_Legal_Info", engine)
        df1.to_sql("Relationship_Information", engine)
...
# DEFINCE A FUNCTION TO CREATE THE DOCUMENTATION OF THE DATABASE WHICH HELPS WITH THE UNDERSTANDING DATABASE AND ITS
# STRUCTURE
...
# Example usage
firstStatement = (
    """Summary of relationships between nodes with Relationship_StartNode_NodeID and Relationship_EndNode_NodeID,
    Relationship_RelationshipType, periods, qualifiers, quantifiers, and registration details.
    Relationship_EndNode_NodeID is the foreign key in the table Relationship_Information. LEI column is the primary
    key in the table Entity_Legal_Info which is the foreign key Relationship_EndNode_NodeID column in the table
    Relationship_Information. Relationship_RelationshipType column contains the information about the relation
    between the given company name. There are 4 type of relations in Relationship_RelationshipType column
    'IS_DIRECTLY_CONSOLIDATED_BY', 'IS_ULTIMATELY_CONSOLIDATED_BY', 'IS_FUND-MANAGED_BY' and 'IS_FUND-MANAGER'.
    Please find the description of all the relations below: 'IS_ULTIMATELY_CONSOLIDATED_BY' - is the ultimate child
    of given company. 'IS_DIRECTLY_CONSOLIDATED_BY' - is the direct child of given company. 'IS_FUND-MANAGED_BY' - is
    a relation who is managing the fund of given company. 'IS_FUND-MANAGER' - is a relation who's funds are managed
    by given company."""
)
secondStatement = (
    """When user asks for the company name like CITIGROUP INC. then use the LEI ID in the search query and use the
    relationship_endnode_nodeid within the relationship table to search for the results. Then use the query like
    mentioned in the below train query"""
)
thirdStatement = (
    """When a user asks for the company name like CITIGROUP INC. then use the LEI ID in the search query and use the
    relationship_endnode_nodeid within the relationship table to search for the results. Then use the query mentioned
    in the below train query. And if user also asks the details of the children or parents of the name like CITIGROUP
    INC. then please use the relationship_endnode_nodeid and search in the 'Entity_Legal_Info' table for that LEI and
    show the maximum data available in the database."""
)
fourthStatement = (
    """When constructing SQL queries for the database, ensure that all input values related to
    'relationship_relationshipstatus' are converted to uppercase. For example, if the user asks "how many records are
    related to Citigroup Inc whose relationship status is inactive", the query should convert 'inactive' to
    'INACTIVE'. Use the following SQL template as a guide:
...
# prepareVannaDocumentation("When a user asks for the company name like CITIGROUP INC. which is used in
# Entity.LegalName  then use the LEI ID in the search query and use the relationship_endnode_nodeid within the
# relationship table to serach for the results. Then use the query mentioned in the below train query")
